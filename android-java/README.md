...
The configuration DSL is Kotlin only, so this app uses `RumBuilder` from the `core` module and builds the OTLP exporters itself.

## Run it

1. Open `app/src/main/java/com/example/androidjavademo/DemoApplication.java` and replace `<region>` and `<your-ingestion-key>` with your
...
Filter on `service.name = 'sample-android-java'` in the Traces, Logs and Metrics explorers.
...
- Uninstall with `adb uninstall com.example.androidjavademo`.
